...
연구자료 수집·repair·accept
  ...
  -> 첫 LLM 호출 전에 공통·분야별 두뇌와 뉴스에 관련된 지식 준비
  -> GPT 1회: 두뇌와 오늘 뉴스를 함께 읽고 해석·장전 판단
...
첫 해석부터 저장된 두뇌 지식을 제공한다. 뉴스에서 직접 만든 local query와
미리 합성된 공통·분야별 지식을 사용하며, 두뇌 없이 생성한 첫 가설을
기억 검색의 전제조건으로 삼지 않는다. open-world는 새 사건·후보를 배제하지
않는다는 뜻이지 과거 지식을 보지 않는다는 뜻이 아니다.

정상 일일 GPT 요청은 한 번이다. 뉴스 해석과 최종 판단을 같은 요청에서
수행한다. 형식 오류가 발생한 경우에만 최대 한 번의 보정을 허용한다.
다운로드된 이전 goal의 정상 2회/최대 4회 계약은 이 사용자 정정으로 대체한다.

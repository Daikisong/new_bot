아래 프롬프트를 **하루치 뉴스 CSV와 함께 GPT Web Pro 연구 세션에 그대로 넣으면 돼.** 파일명이나 날짜가 잘못돼 있어도 CSV 내부 시각을 기준으로 판정하도록 만들었고, 연구 결과가 Codex의 `research brain`에 바로 들어갈 수 있게 기계용 산출물까지 강제했어.

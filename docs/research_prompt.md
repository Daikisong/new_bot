...
direct_ingest_contract.json.hard_gate_summary.training_provenance_closure_status == "passed"
direct_ingest_contract.json.hard_gate_summary.training_eligible_empty_provenance_count == 0
direct_ingest_contract.json.hard_gate_summary.training_eligible_unresolved_source_count == 0
...
training_eligible=true record가 provenance_source_ids=[] 또는 source까지 닫히지 않은 fact/inference를 가짐
```
...
```json
{
  ...
  "source_fact_ids": ["FACT-..."],
  "source_inference_ids": ["INF-..."],
  "provenance_source_ids": ["SRC-NEWS-ROW-..."],
  "outcome_audit_ids": [],
  "sample_weight": 1.0,
  "payload": {}
}
...
`training_eligible=true`인 예시에 빈 `source_fact_ids`, `source_inference_ids`, `provenance_source_ids`를 쓰지 않는다. 출처가 아직 닫히지 않았으면 해당 record는 예시에서도 `training_eligible=false`, `sample_weight=0.0`, `training_exclusion_reason="missing_provenance_source_ids"`로 표시한다.
...
### 14.0-B training provenance closure hard lock

`training_eligible=true` record는 사람이 읽는 본문 근거가 아니라, 기계가 따라갈 수 있는 source closure를 가져야 한다. 최종 `brain_delta.jsonl`에서 `training_eligible=true`인 모든 line은 다음 중 하나 이상의 경로로 `source_ledger.jsonl`까지 닫힌다.

```text
source_fact_ids -> fact_ledger_blind.source_ids -> source_ledger.source_id
source_inference_ids -> inference_ledger_blind.fact_ids -> fact_ledger_blind.source_ids -> source_ledger.source_id
record.provenance_source_ids -> source_ledger.source_id
postmortem fact/inference ledger -> source_ledger.source_id
```

`record.provenance_source_ids`는 위 경로로 resolved된 source id를 top-level에 materialize한 값이다. 본문 문장, 후보명, ticker명, 시장 해설, 또는 "위 뉴스에서 봄" 같은 자연어 참조는 closure로 인정하지 않는다.

최종 Markdown에는 `record_provenance_closure_audit.jsonl` block을 정확히 1회 포함한다. 각 row는 최소 다음 필드를 가진다.

```json
{
  "record_id": "BD-...",
  "record_type": "supervised_direct_event_case",
  "training_eligible_before_closure": true,
  "training_eligible_after_closure": true,
  "source_fact_ids": ["FACT-..."],
  "source_inference_ids": ["INF-..."],
  "resolved_provenance_source_ids": ["SRC-NEWS-ROW-..."],
  "closure_path": "source_inference_ids -> inference_ledger_blind -> fact_ledger_blind -> source_ledger",
  "closure_status": "CLOSED",
  "downgrade_reason": null
}
```

source까지 닫히지 않으면 해당 record를 버리지 말고 다음처럼 학습 대상에서 내린다.
...
```json
{
  ...
  "closure_status": "DOWNGRADED_NOT_TRAINING",
  "downgrade_reason": "missing_provenance_source_ids"
}
```

이 downgrade는 연구 record를 보존하기 위한 안전장치다. downgrade된 record는 `brain_delta.jsonl`에 남길 수 있지만 반드시 `training_eligible=false`, `sample_weight=0.0`, `training_exclusion_reason="missing_provenance_source_ids"`여야 한다.

검증식:

```text
training_eligible_record_count == training_eligible_provenance_closed_count
training_eligible_empty_provenance_count == 0
training_eligible_unresolved_source_count == 0
training_eligible_missing_fact_or_inference_path_count == 0
training_eligible_source_id_not_in_source_ledger_count == 0
record_provenance_closure_audit_count == brain_delta_actual_record_count
record_provenance_closure_audit_missing_record_count == 0
record_provenance_closure_audit_orphan_record_count == 0
```

`validation_report.json`과 `direct_ingest_contract.json.record_import_manifest`는 위 숫자를 mirror한다. 하나라도 실패하면 먼저 `brain_delta.jsonl`과 closure audit를 수리하고 final Markdown을 다시 열어 재파싱한다. 수리 후에도 `training_eligible=true` record가 source까지 닫히지 않으면 `ACCEPT_FULL`, `brain_eligible=true`, `direct_brain_ingest_ready=true` 금지다.
...
training_provenance_closure_status_verified
training_eligible_empty_provenance_count_zero_verified
training_eligible_unresolved_source_count_zero_verified
record_provenance_closure_audit_count_verified
...
accept_full_allowed = (
    ...
    and training_provenance_closure_status == "passed"
    and training_eligible_empty_provenance_count == 0
    and training_eligible_unresolved_source_count == 0
    and record_provenance_closure_audit_count == brain_delta_actual_record_count
...
```json
{
  ...
  "training_eligible_empty_provenance_count": 0,
  "training_eligible_unresolved_source_count": 0,
  "record_provenance_closure_audit_count": 0,
...
2a. <!-- NSLAB:BEGIN record_provenance_closure_audit.jsonl --> ... <!-- NSLAB:END record_provenance_closure_audit.jsonl --> block을 정확히 1회 찾는다.
...
8a. training_eligible=true record의 empty provenance, unresolved source, missing fact/inference path count를 계산한다.
8b. record_provenance_closure_audit.jsonl의 record_id set이 brain_delta.jsonl record_id set과 같은지 계산한다.
...
```json
{
  ...
  "record_provenance_closure_audit_count": 0,
  "record_provenance_closure_audit_jsonl_sha256": "",
  "training_provenance_closure_status": "passed",
  "training_eligible_empty_provenance_count": 0,
  "training_eligible_unresolved_source_count": 0,
...
record_import_manifest.record_provenance_closure_audit_count == record_import_manifest.actual_brain_delta_record_count
record_import_manifest.record_provenance_closure_audit_jsonl_sha256 == bundle_manifest.block_hashes["record_provenance_closure_audit.jsonl"].sha256
record_import_manifest.training_provenance_closure_status == "passed"
record_import_manifest.training_eligible_empty_provenance_count == 0
record_import_manifest.training_eligible_unresolved_source_count == 0
...
```json
{
  ...
  "episode_id": "",
  "required_block_count": 33,
  "required_blocks_present_count": 33,
  ...
  "block_hashes": {
    ...
    "record_provenance_closure_audit.jsonl": {"sha256": "", "byte_size": 0},
...
training_eligible=true record의 provenance_source_ids가 비어 있거나 source_ledger까지 닫히지 않음
record_provenance_closure_audit.jsonl block이 누락되거나 brain_delta record 수와 불일치
...
accept_full_allowed = (
    ...
    and training_eligible_empty_provenance_count == 0
    and training_eligible_unresolved_source_count == 0
    and record_provenance_closure_audit_count == brain_delta_record_count
...
```json
{
  ...
  "event_ids": ["EVT-..."],
  "observation_ids": ["OBS-..."],
  "screening_ids": ["SCREEN-..."],
  "combined_fact_ids": ["FACT-..."],
  "combined_inference_ids": ["INF-..."],
  "event_count": 0,
...
```json
{
  ...
  "causal_chain_inference_ids": ["INF-..."],
  "cutoff_fact_ids": ["FACT-..."],
  "cutoff_inference_ids": ["INF-..."],
  "outcome": {},
...
```json
{
  ...
  "semantic_basis_fact_ids": ["FACT-..."],
  "semantic_basis_inference_ids": ["INF-..."],
  "postmortem_fact_ids": ["FACT-POST-..."],
  "semantic_basis_source_ids": ["SRC-NEWS-ROW-..."],
  "actual_outcome": {},

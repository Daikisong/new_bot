...
def test_provenance_audit_rejects_incompatible_final_synthesis_required_inputs(
    tmp_path: Path,
) -> None:
    (tmp_path / "predictions").mkdir()
    (tmp_path / "reports").mkdir()
    ...
    run_report_path.write_text(report_text, encoding="utf-8")
    final_payload = {
        "required_inputs": ["current_news", "extra_debug_input"],
        "current_news": ["pre-cutoff news"],
        "extra_debug_input": {"unexpected": True},
    }
    final_context_path = (
        tmp_path
        / "runs"
    ...
    assert not result["passed"]
    assert (
        "2030-01-10.json: final_synthesis_context required_inputs incompatible"
    ) in result["findings"]
...
def test_provenance_audit_rejects_final_synthesis_future_record_ids(
    ...
    future_record = _brain_record_for_sweep_audit(
        "REC-final-future",
        episode_id="NSLAB-20300109-FINAL-CONTEXT",
        available_from=datetime(2030, 1, 10, 9, 30, 0, tzinfo=KST),
    )
    _store_brain_records_for_sweep_audit(tmp_path, [future_record])
    final_payload = {
        "required_inputs": ["current_news"],
        "current_news": ["pre-cutoff news"],
        "retrieved_record_ids": ["REC-final-future"],
        "retrieved_records": [{"record_id": "REC-final-future"}],
        "record_level_shard_contributions": [
            {"payload": {"record_ids": ["REC-final-future"]}}
        ],
    }
    final_context_path = (
        tmp_path
    ...
    assert not result["passed"]
    assert (
        "2030-01-10.json: final_synthesis_context retrieved_record_ids exposes "
        "future record: REC-final-future"
    ) in result["findings"]
    assert (
        "2030-01-10.json: final_synthesis_context retrieved_records exposes "
        "future record: REC-final-future"
    ) in result["findings"]
    assert (
        "2030-01-10.json: final_synthesis_context "
        "record_level_shard_contributions exposes future record: REC-final-future"
    ) in result["findings"]


def test_provenance_audit_rejects_final_synthesis_manifest_record_id_mismatch(
    tmp_path: Path,
) -> None:
    (tmp_path / "predictions").mkdir()
    (tmp_path / "reports").mkdir()
    ...
    _store_brain_records_for_sweep_audit(
        tmp_path,
        [
            _brain_record_for_sweep_audit(
                "REC-final-manifest",
                episode_id="NSLAB-20300109-FINAL-MISMATCH",
    ...
    assert not result["passed"]
    assert (
        "2030-01-10.json: final_synthesis_context retrieved_record_ids "
        "does not match context manifest"
    ) in result["findings"]
    assert (
        "2030-01-10.json: final_synthesis_context semantic_retrieval_record_ids "
        "does not match context manifest"
    ) in result["findings"]
    assert (

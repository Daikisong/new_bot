...
```text
execution_protocol_version = nslab.research_daily_population_complete.v8
```
...
### 5.5.1 BLIND hypothesis case

BLIND에 실제 존재한 theme hypothesis마다 outcome snapshot으로 다음을 평가한다.

```text
formation_status
verified upper-limit member count
verified high20 member count
verified high10 member count
member amount concentration
member turnover concentration
actual leader candidates
negative peers
```

테마 membership은 고정 산업 taxonomy로 강제하지 않는다.

BLIND event·후보·archetype과 cutoff 이전 증거를 기반으로 LLM이 구성하며, 근거를 남긴다.

### 5.5.2 retrospective discovered theme

결과 뒤 처음 발견한 theme은 BLIND 적중으로 계산하지 않는다.
...
다음 날부터 잠정 memory로 사용할 수 있으나 `theme_formation forecast hit`가 아니다.
...
### 5.7.1 blind_leader_preference_pair
...
### 5.7.2 candidate_generation_error_pair
...
### 5.7.3 retrospective_population_pair

결과 뒤 새로 만든 pair는 기본적으로 leader training 불가다.

다음이 증명된 경우에만 낮은 신뢰도로 별도 허용한다.
...
```json
{
  ...
  "sample_weight": 0.0,
  "label_quality": "verified | quarantined | no_tradable_row",
  "training_target": "single_event_association",
...
```json
{
  ...
  "blind_hypothesis": {},
  "formation_status": "",
  "verified_upper_limit_tickers": [],
  "verified_high20_tickers": [],
  "verified_high10_tickers": [],
  "peer_universe": [],
  "breadth_metrics": {},
...
```json
{
  "schema_version": "nslab.blind_prediction.v8",
  "episode_id": "",
  "trade_date": "",
  "previous_trade_date": "",
  "window_start": "",
...
```yaml
schema_version: nslab.research_bundle.v8
...
```json
{
  "schema_version": "nslab.research_episode.v8",
  "episode_id": "",
  "trade_date": "",
  "previous_trade_date": "",
  "next_trade_date": "",
...
## 6. BLIND 엔티티 의미 정확도
## 7. 직접 기업뉴스 관측 모집단
## 8. 모든 observation 후보 심사
## 9. 사건 지도
## 10. 오픈월드 최초 분석
## 11. 주도섹터 가설
## 12. 단일뉴스 후보
## 13. 테마 수혜 archetype·후보
## 14. D-1 연속성 후보
## 15. BLIND pairwise 비교
## 16. 최종 장전 관심종목
## 17. BLIND Red-team
## 18. BLIND packet manifest

--- BLIND 봉인 이후 결과 공개 ---

## 19. OUTCOME snapshot 완전성·해시 검증
## 20. Post-seal 엔티티 확정
## 21. 전 시장 상한가·강한 상승 census
## 22. issuer-day 감독학습 모집단
## 23. 직접뉴스 event-level 감독학습 모집단
## 24. 후보 생성·순위 오류
## 25. 주도섹터 형성 연구
## 26. 수혜주 발견 연구
## 27. 엄격한 대장 선택 연구
## 28. 후보 실패·부정 대조군
## 29. 행·엔티티·ticker binding 오류
## 30. 학습 적격성 매트릭스
## 31. Brain Delta 요약
## 32. 다음 연구 질문
## 33. 출처·데이터 한계
...
```json
{
  "schema_version": "nslab.bundle_manifest.v8",
  "episode_id": "",
  "trade_date": "",
  "created_at": "",
  "execution_protocol_version": "nslab.research_daily_population_complete.v8",
...
6. entity gate 완성
7. 모든 직접 observation 후보 screening 완성
8. 사건·섹터·후보·continuation·pairwise·red-team 완성
9. BLIND 패킷 전체 파일 저장·해시·봉인
10. 봉인 재검증
11. 그 뒤 outcome snapshot(D) 다운로드·검증
12. 전 시장 outcome ledger·winner census 생성
13. post-seal entity resolution
14. issuer-day 집계와 event-level weight 생성
15. 직접뉴스·테마·수혜주·leader·negative control 연구
16. 적격성 결정
17. Brain Delta 생성
18. 모든 JSON·JSONL·참조·해시 검증
19. 단일 Markdown bundle 조립
20. 실제 다운로드 가능한 MD 파일 생성
```

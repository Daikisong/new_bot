...
mod.RUN_STAMP = '20260715T045906KST'
mod.RUN_ID = 'nslab_run_20260715T045906KST_20220824'
...
def run_range(args: argparse.Namespace) -> None:
    ...
    if metadata.get('run_id') != mod.RUN_ID:
        raise RuntimeError(f"prepared run mismatch: {metadata.get('run_id')}")
    ...
    def process(batch: list[dict], label: str) -> None:
        try:
            ...
            for row in batch:
                ...
                normalized['semantic_review_protocol'] = 'FULL_TITLE_BODY_E1_E2_E3_ADJUDICATION_NO_KEYWORD_SHORTCUT_CURRENT_RUN_MICRO_RANGE'
    ...
    for batch_index, batch in enumerate(mod.batches(model_inputs, max_items=10, max_chars=85000), 1):
        process(batch, f'CURRENT_MICRO_FULL_ROW_{start:04d}_{end:04d}_B{batch_index:02d}')
    ...
    mod.write_json(output / f'receipt_range_{start:04d}_{end:04d}.json', {
        'schema_version': 'nslab.semantic_review_micro_range_receipt.v2',
        'run_id': mod.RUN_ID,
        ...
        'status': 'FULL_ROW_SEMANTIC_REVIEW_CURRENT_RUN_MICRO_RANGE_COMPLETE',

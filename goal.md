맞아. **AGENTS.md에는 하드코딩 금지와 작업 규칙만 넣고, 몇 년치 연구지식은 별도의 버전형 `research brain`으로 관리해야 해.** Codex는 저장소의 `AGENTS.md`를 매 실행 전에 읽지만 기본 결합 한도가 있으므로 거대한 연구자료를 넣는 장소가 아니야. 반복 워크플로는 저장소의 `.agents/skills`에 둘 수 있어. ([OpenAI Developers][1])

아래를 **새 뉴스 스캘핑 프로젝트 저장소의 Codex에 그대로 한 번에 입력**하면 돼.

...
research_prompt_revision = nslab.research_prompt.direct_ingest_gold.v16_20241213_regression_locked
revision_goal = 사람 후검수 없이 자동 import-ready ACCEPT_FULL bundle을 생성하되, phase 오염·다운로드 계층 혼동·validator 셀프채점·상장사 의미 오결속·근거 없는 catalyst/mechanism 생성·final filler 후보·20241213형 common-word false positive를 최상단 실행 차단기로 막는다.
```
...
final_common_word_false_positive_count == 0
...
semantic_firewall_fixture_all_passed
...
────────────────────────────────────────
SEMANTIC COCKPIT LOCK v16 — 20241213 결과 기반 final 후보 의미 검증
────────────────────────────────────────

이 섹션은 20241213 pre-v16 연구 결과에서 실제로 드러난 오류를 막기 위한 최상단 의미 차단기다. PHASE가 깨끗해도 이 섹션을 통과하지 못하면 gold가 아니다.

20241213 결과의 핵심 문제는 phase보다 **의미 결속**이었다.

```text
오로라 자연현상 기사 → 상장사 오로라로 결속
DSR 대출규제 약어 → 상장사 DSR로 결속
ALL NEW 상품명 → 상장사 NEW로 결속
제품 제조사 코스맥스 단순언급 → BIO_STAGE_ADVANCE로 승격
슈퍼개미의 알테오젠 보유/투자 언급 → CONTRACT_ORDER처럼 해석
삼성바이오로직스 행사 참석자 목록 → BIO_STAGE_ADVANCE로 승격
현대로템 세미나 발표 → CONTRACT_ORDER로 승격
신성델타테크 기관 순매수 표 → PRODUCT_COMMERCIALIZATION로 승격
supporting_inference_ids가 비어 있는데 economic_mechanism 템플릿 생성
final_watchlist를 20개로 채우기 위해 약한 후보 padding
```
...
각 mention은 반드시 아래 중 하나로 분류한다.

```text
ARTICLE_MAIN_SUBJECT
DIRECT_SECONDARY_ACTOR_WITH_LOCAL_ACTION
EXPLICIT_TICKER_CODE_MATCH
ANALYST_TARGET
...
단, `DIRECT_SECONDARY_ACTOR_WITH_LOCAL_ACTION`은 같은 문장 또는 인접 1문장 안에 해당 issuer가 수행하거나 해당 issuer에 직접 귀속되는 경제 predicate가 있어야 한다.
...
## SC2. high-risk literal denylist는 final 승격 전 local proof를 요구한다

아래 이름은 P snapshot에 동일 회사명이 있어도 일반명사·약어·브랜드·형용사·자연현상과 충돌하기 쉽다. 이 목록은 폐쇄형 blacklist가 아니라 hard fixture seed다.
...
high-risk literal은 다음 중 하나가 같은 row 안에 없으면 `ACCEPT_FOR_SCREENING` 금지다.
...
## SC3. quote_role과 catalyst_type 허용 매트릭스

각 final candidate의 primary fact는 `quote_role`, `material_fact_class`, `catalyst_type`을 가진다.
...
```text
CONTRACT_ORDER
  허용 fact_class: CONTRACT_SIGNED, ORDER_RECEIVED, SUPPLY_AGREEMENT, PROJECT_AWARDED

PRODUCT_COMMERCIALIZATION
  허용 fact_class: PRODUCT_LAUNCHED_BY_ISSUER, PRODUCT_COMMERCIALIZATION_BY_ISSUER, SERVICE_RELEASE_BY_ISSUER

BIO_STAGE_ADVANCE
  허용 fact_class: REGULATORY_APPROVAL, CLINICAL_STAGE_ADVANCE, LICENSE_OR_TECH_TRANSFER_WITH_RIGHTS, GOVERNMENT_PROJECT_SELECTED

CAPITAL_POLICY
  허용 fact_class: DIVIDEND, BUYBACK, SHARE_CANCELLATION, RIGHTS_ISSUE, THIRD_PARTY_ALLOCATION, MERGER_OR_SPINOFF, STAKE_SALE_OR_CONTROL_CHANGE

ANALYST_BRIDGE
  허용 fact_class: ANALYST_NUMERIC_EARNINGS_BRIDGE

CONTINUATION_EXPLICIT
  허용 fact_class: EXPLICIT_MARKET_STATE_NOTICE
```
...
## SC4. economic_mechanism은 SUPPORTED inference로 닫는다
...
```json
{
  ...
  "issuer_role_anchor_type": "ARTICLE_MAIN_SUBJECT | DIRECT_SECONDARY_ACTOR_WITH_LOCAL_ACTION | EXPLICIT_TICKER_CODE_MATCH | ANALYST_TARGET | FORMAL_DISCLOSURE_ISSUER",
...
## SC5. final_semantic_audit.jsonl 필수
...
## SC6. 20241213 regression fixtures

validator는 실제 episode와 별도로 다음 synthetic fixture를 매번 실행하고 `validation_report.json`에 결과를 남긴다.

```jsonl
{"fixture_id":"R13-001","literal":"오로라","quote":"캐나다관광청 \"올겨울은 오로라 관측 최적기\"","expected":"REJECT_PLACE_OR_NATURE_PHENOMENON"}
{"fixture_id":"R13-002","literal":"DSR","quote":"총부채원리금상환비율(DSR) 시행","expected":"REJECT_POLICY_ACRONYM_OR_COMMON_NOUN"}
{"fixture_id":"R13-003","literal":"NEW","quote":"ALL NEW 새우초밥","expected":"REJECT_PRODUCT_ADJECTIVE_OR_BRAND_ONLY"}
{"fixture_id":"R13-004","literal":"코스맥스","quote":"제품의 제조사는 코스맥스이다.","expected":"REJECT_MANUFACTURER_ONLY_FOR_FINAL"}
{"fixture_id":"R13-005","literal":"알테오젠","quote":"슈퍼 개미가 알테오젠에 대규모 투자를 했다","expected":"REJECT_INVESTOR_HOLDING_MENTION_ONLY_FOR_FINAL"}
{"fixture_id":"R13-006","literal":"삼성바이오로직스","quote":"삼성바이오로직스 등 관계자 20여 명이 참석했다","expected":"REJECT_ATTENDEE_LIST_ONLY_FOR_FINAL"}
{"fixture_id":"R13-007","literal":"신성델타테크","quote":"기관 코스닥에서 신성델타테크 등 순매수","expected":"REJECT_MARKET_FLOW_TABLE_MEMBER_FOR_FINAL"}
{"fixture_id":"R13-008","literal":"현대로템","quote":"현대로템 실장은 개발 현황과 인력 양성 방안에 대해 발표했다","expected":"REJECT_PRESENTATION_ONLY_FOR_FINAL"}
{"fixture_id":"R13-009","literal":"하이로닉","quote":"하이로닉 기업설명회(IR) 개최","expected":"REJECT_IR_CALENDAR_ONLY_FOR_FINAL"}
```

ACCEPT_FULL 조건:

```text
r13_regression_fixture_count >= 9
r13_regression_fixture_failure_count == 0
```

## SC7. DART/공시 HTML body cleanup
...
## SC8. semantic gate summary counters
...
r13_regression_fixture_count
r13_regression_fixture_failure_count
...
final candidate의 entity_role이 ARTICLE_MAIN_SUBJECT/DIRECT_SECONDARY_ACTOR_WITH_LOCAL_ACTION/EXPLICIT_TICKER_CODE_MATCH가 아님
final candidate가 mention-only, homonym collision, generic word collision, attendee list, market table에서 발생함
...
3. final candidate reason이 약하거나 semantic gate를 위반함
   ...
   - 오로라/DSR/NEW/대상/선진 등 contextual false positive는 issuer_scoped=false로 내리고 final에서 제거한다.
...
semantic_regression_20241213_test_count_min_9
semantic_regression_20241213_failure_count_zero
